...
import { FaqList } from "@/app/_api/api";
...
export default function Faq() {
  ...
  useEffect(() => {
    setC(JSON.parse(sessionStorage.getItem("citizen")));
  },[]);
  ...
  useEffect(() => {
    FaqList().then((dataArray) => {
      setAllData(dataArray);
    });
    // if(allData?.faq?.length > 0){
    //     let tp = ((parseFloat(allData?.faq?.length)) / (parseInt(showItemLength)));
    //     console.log('tp ',tp);
        ...
        <div className="py-[13px] lg:py-[22.7px] px-[8px] lg:px-[26px]">
          ...
          <div className="flex lg:space-x-[18px]">
            <div className="lg:w-[70%]">
              ...
              {allData?.faq
                ?.slice(startWith, startWith + showItemLength).map((item, index) => (
                  <div
                    key={index}
                    className={`w-full ${
                  ...
                  <div className="py-4">
                    <div className="flex justify-end lg:justify-end space-x-3 lg:space-x-8 items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {startWith + showItemLength >= allData?.faq?.length ? (
                          <>
                            <div>
                              <img
                    ...
                    <div className="relative inline-block text-left">
                      ...
                      {paginateOptionShow && (
                        <div
                          onMouseEnter={() => setPaginateOptionShow(true)}
                          onMouseLeave={() => setPaginateOptionShow(false)}
                          className="absolute top-3 right-0 z-10 mt-2 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                          role="menu"
                          aria-orientation="vertical"
                          aria-labelledby="menu-button"
                          tabIndex={-1}
