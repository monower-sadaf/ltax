...
import { GetParentOrganization, SearchParentOrganizationByUsername, SendRequestToParentOrg } from "@/app/_api/api";
...
const Home = () => {
  ...
  useEffect(() => {
    let orgData = JSON.parse(org.organizationinfo);
    GetParentOrganization(orgData?.id).then((res => {
      console.log(res?.data);
      setParentOrg(res?.data)
    }))
  }, [])
  ...
  const sendRequest = (id) => {
    const orgInfo = JSON.parse(org?.organizationinfo);
    let input = {
      id: id,
      org_id: orgInfo?.user_id,
      child_username: orgInfo?.username,
      child_name: orgInfo?.org_name,
      child_phone: orgInfo?.org_mobile,
      username: searchParams?.parentOrg
    }

    SendRequestToParentOrg(input).then(res => {
      if (res?.data?.length < 1) {
                    ...
                    <tbody>
                      {data?.map((item, index) => (
                        <tr key={index} className="h-10">
                          ...
                          <td
                            className={
                              searchParams === item?.phone && "bg-yellow-200"
                            }
                          >
                            {en2bn(item?.phone)}
                          </td>
                          ...
                          <td className="flex flex-col items-center">
                            <button onClick={() => sendRequest(item?.user_id)}
          ...
          {parentOrg?.length > 0 ? (
            <div className="w-full overflow-x-auto my-4">
              <table className="w-full text-center">
                ...
                <tbody>
                  {parentOrg?.map((item, index) => (
                    <tr key={index} className="h-10">
                      <td>{item?.name}</td>
                      <td>{item?.name}</td>
                      <td>
                        {en2bn(item?.parent_org_phone)}
                      </td>
                      <td>{item?.parent_org_username}</td>
                      <td>{item?.address}</td>
                      {/* <td className="flex flex-col items-center">
                        <button onClick={() => sendRequest(item?.user_id)}
                          className="items-center flex items-center bg-[#12633D] text-white text-12 leading-[12.14px] lg:text-14 lg:leading-[14.16px] rounded-md px-3 py-2 space-x-2"
